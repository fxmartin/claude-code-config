...
# The whole document is fenced in a managed-region marker pair (Story
# 12.2-001) so `render --out` can splice it in place without clobbering any
# out-of-band history that surrounds it:
#
#   <!-- BEGIN SDLC LEDGER ... -->
#   # Build Progress
#   <!-- AUTOGENERATED ... -->
...
# Managed-region markers (Story 12.2-001). The auto-generated document is
# fenced between these two comment lines. On `--out`, only the content
# *between* the markers is regenerated — anything outside survives verbatim
# so hand-maintained / markdown-workflow history is never clobbered. The
# marker strings are matched by substring, so the trailing parenthetical may
# evolve without breaking older files.
SDLC_MANAGED_BEGIN='<!-- BEGIN SDLC LEDGER (auto-generated by sdlc-state render — do not edit this region) -->'
SDLC_MANAGED_END='<!-- END SDLC LEDGER -->'
...
# Gate section. The whole document is fenced between the managed-region
# markers (Story 12.2-001) so the `--out` splice can replace exactly this
# block while preserving any out-of-band content around it. The trailing
# newline is intentional — POSIX text files end in a newline.
render_document() {
    printf '%s\n\n' "${SDLC_MANAGED_BEGIN}"
    render_header
    render_historical
...
render_subcommand() {
    ...
    # Atomic write: assemble the next document into <out>.tmp, then rename.
    # The rename is atomic on POSIX filesystems so concurrent readers either
    # see the old file or the new one — never a partial render.
    ...
    # Render the managed block (markers included) into its own tmp first.
    local managed_tmp="${out_path}.managed.tmp"
    render_document >"${managed_tmp}"

    # Non-destructive merge (Story 12.2-001):
    #   * existing file WITH a managed region  -> splice in place, keep the rest
    #   * existing file WITHOUT markers          -> preserve it, append the region
    #   * no existing file (greenfield)          -> write the managed block as-is
    # Each branch writes into <out>.tmp so the rename remains atomic.
    if [ -f "${out_path}" ] && has_managed_region "${out_path}"; then
        splice_managed_region "${out_path}" "${managed_tmp}" >"${tmp_path}"
    elif [ -f "${out_path}" ]; then
        { cat "${out_path}"; printf '\n'; cat "${managed_tmp}"; } >"${tmp_path}"
    else
        cp "${managed_tmp}" "${tmp_path}"
    fi
