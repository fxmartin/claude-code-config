...
Generate a `CLAUDE.md` with foundational sections only. Deep sections (testing strategy, CI/CD pipeline, data model, etc.) will be filled after `/dev:brainstorm` and `/generate-epics`.
...
<!-- Populated after /dev:brainstorm and /generate-epics -->
...
This is the **handoff file** that `/dev:brainstorm` reads to skip already-answered questions and pre-fill context.

```markdown
# Project Seed — <project-name>

> Auto-generated by `/project-init` on <YYYY-MM-DD>. Consumed by `/dev:brainstorm`.
...
- **Next step**: "Run `/dev:brainstorm` to define product requirements. It will pick up your PROJECT-SEED.md automatically."
