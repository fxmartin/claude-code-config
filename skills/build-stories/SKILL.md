...
- **Flags**:
  ...
  - `--sequential` — disable parallel mode and build stories one at a time (parallel worktree-isolated builds are the default)
...
### Phase 4b: Parallel Worktree Scheduling (default, unless `--sequential`)
...
**Dry-run in parallel mode (default)**: Show the cohort groupings in the display table with `--- Cohort N ---` separator rows and indicate worktree isolation will be used.
...
### Phase 5 — Parallel Worktree Mode (default, unless `--sequential`)

Unless `--sequential` is set, execute each cohort using batch-per-stage parallelism. **FOR EACH cohort** computed in Phase 4b:
...
### Phase 5 — Sequential Mode (only when `--sequential`)
