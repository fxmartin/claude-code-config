...
| `sdlc queue list` · `sdlc queue add` · `sdlc queue cancel <id>` · `sdlc queue requeue <id>` · `sdlc queue prioritise <id> <class>` | Inspect and manage the host-level job queue (`$XDG_STATE_HOME/sdlc/queue.db`, `SDLC_QUEUE_PATH`-overridable) — every job on the host across repos, any state, priority, budget, age, and the PR/MR a `parked` job is waiting on, plus a banner when the queue is paused on a shared rate-limit window (`--json` emits `{pause, jobs}`). Claim order is priority class then age; an omitted `--priority` is derived (a `fix` outranks a `build`, a `--label bug` outranks an `--label enhancement`, mirroring `fix all`). `prioritise` moves a job between classes and re-stamps its budget — which is also how `SDLC_QUEUE_MAX_FIX_ROUNDS` / `SDLC_QUEUE_WALL_CLOCK_MINUTES` reach a job that is already enqueued, since those are read when a budget is stamped (`add`, `--enqueue`, `prioritise`) and never at drain time. `requeue` re-arms a `blocked`, `parked`, `needs_attention`, `failed` or `cancelled` job with its frozen options intact; one that already opened a run comes back to be *resumed*, not restarted, with a fresh wall clock and one more allowance of fix rounds |
| `sdlc queue run [--slots N] [--follow] [--approval-poll-interval S]` | Drain the host queue in the foreground: claim each job under a renewable lease and run it with the existing `build`/`fix` machinery as a subprocess. Never two jobs in one repo, never a job whose *recorded* investigated files overlap a pending peer's in the same repo (a footprint is only known once a run has frozen its plan, so this holds a returning job, not two never-run ones), never more than `--slots` agent slots host-wide (default 2). Each job is held to the budget frozen on it at enqueue — max `bugfix` rounds (read from the run's ledger) and a wall-clock cap per launch; exceeding either stops the job and parks it `needs_attention` with notify, and `sdlc queue requeue` is the exit. That is the queue's outer breaker — the pipeline's own per-story `MAX_BUGFIX_ATTEMPTS` is a separate, untouched knob. One Max window is shared by every repo, so a job that parks `RATE_LIMITED` pauses **all** dispatch until the reset, then the queue resumes the parked job itself — one pause notification and one resume, not one per repo. Ctrl-C stops its jobs and hands the leases back (exit 130); the next run reclaims any lapsed job and re-enters it via `sdlc resume`, never from scratch. A run that stops `AWAITING_APPROVAL` is recorded `parked` with its PR/MR and holds no slot — the queue re-reads that change request read-only every `--approval-poll-interval` seconds (default 300, bounded 30–3600) and, the moment it carries `risk-approved` or an approving review, resumes the run so the controller performs the merge and records DONE. A PR merged by hand is reconciled instead; one closed without merging fails the job |
...
./install.sh --core                                      # symlinks the codex/qwen/opencode adapters onto PATH
...
| **CI gate at merge** | Before merging — in both `sdlc build` and `sdlc fix` — the controller polls the PR/MR's CI status for the current head and refuses to merge on failure or a still-running pipeline, re-reading after every bugfix push (no-CI behaviour set by `--ci-gate-no-ci=allow\|deny`). This repo's CI carries the security jobs: gitleaks secret detection (fails on any finding) and a supply-chain scan | [`docs/security-gates.md`](docs/security-gates.md) |
