...
| `sdlc queue list` · `sdlc queue add` · `sdlc queue cancel <id>` · `sdlc queue requeue <id>` · `sdlc queue prioritise <id> <class>` | Inspect and manage the host-level job queue (`$XDG_STATE_HOME/sdlc/queue.db`, `SDLC_QUEUE_PATH`-overridable) — every job on the host across repos, any state, priority, and age. `requeue` re-arms a parked (`blocked`), `failed` or `cancelled` job with its frozen options intact; one that already opened a run comes back to be *resumed*, not restarted |
