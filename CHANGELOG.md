...
- fix(repair): stop managing skills as one symlink; dedupe bats test (#712)
...
- fix(install): link skills per entry so foreign skills survive --core (#707)
...
- fix(build): retry cr open before review when resume finds no cr (#704)
...
- fix(build): route merge and reconcile via the declared forge (#699) (#703)
...
- fix(preflight): raise the default ceiling and stop calling a timeout red (#689)
...
- feat(install): symlink codex instructions into ~/.codex (#690)
...
- feat(install): arch linux / omarchy support (#687)
...
- fix(resume): re-register the run in the host registry with its own pid (#684)
...
- fix(review-packet): degrade to a summary tier instead of dropping oversized packets (#676)
...
- feat(development-queue): queue store and `sdlc queue` verbs (#32.1-001)
...
- feat(controller-ux): dashboard sidebar — filter runs by (#19.2-003)
...
- feat(harness): refuse host-auth roles on an undenied harness (#654) (#658)
...
- fix(repair): refuse unsafe repair roots and missing artifact sources (#651)
...
- fix(risk-gate): evaluate detector and policy from trusted base ref (#649)
...
- feat(harness-benchmarking-baseline): benchmarking method (#31.3-002) (#631)
...
- feat(harness-benchmarking-baseline): a cost figure that (#31.2-003) (#629)
...
- feat(harness-benchmarking-baseline): recorded (#31.3-001) (#628)
...
- feat(harness-benchmarking-baseline): honest token (#31.2-002) (#627)
...
- feat(harness-benchmarking-baseline): scoreboard provenance (#31.1-002) (#626)
...
- feat(harness-benchmarking-baseline): stall-adjusted (#31.2-001) (#623)
...
- fix(adapters): correct the opencode example and strip ansi (#624)
...
- fix(sdlc): write Status: Done back to story markdown on landing (#603)
...
- fix(issues): parse gitlab work-item issue urls
...
- fix(sdlc): make stage retries collision-safe (#589) (#592)
...
- feat(notify): render human-readable telegram lifecycle messages (#584)
...
- fix(dashboard): surface fix-run investigation stage and status (#567)
...
- fix(controller): re-probe live api before honouring parked reset (#564) (#566)
...
- fix(resume): don't block on deps already done in the epic (#536) (#538)
...
- fix(skills): give the codex wrapper a guarded sdlc fallback (#532)
...
- feat(empirical-estimation): route model escalation on (#28.3-001) (#526)
...
- feat(empirical-estimation): ledger-vs-logs reconciliation (#28.1-001) (#521)
...
- fix(ledger): record original session usage on reask + recovery model (#482)
...
- feat(performance-token-optimization): pre-baked review (#27.3-003)
...
- feat(performance-token-optimization): deterministic (#27.3-001)
...
- feat(performance-token-optimization): fix-issue mirror — (#27.2-003)
...
- feat(performance-token-optimization): ledger stall telemetry (#27.3-004) (#472)
...
- feat(performance-token-optimization): split fix-issue (#27.1-004) (#471)
...
- feat(performance-token-optimization): align fix-issue with Balanced profile (#27.1-001) (#467)
...
- feat: sdlc fix verb — single-issue controller pipeline (#436, pr 1/3) (#461)
...
- fix: harness-aware pre-dispatch cost estimate (#427) (#459)
...
- fix(lens): fail closed on malformed findings instead of dropping them (#458)
...
- fix(lens): ship the overengineering-lens.sh wrapper the config invokes (#457)
...
- feat: dispatch the over-engineering lens as a review advisory (#445) (#455)
...
- fix(harness): label default-slot preflight and log per-role routing (#454)
- fix(contracts): drop unread security_status from coverage contract (#453)
...
- fix: eval harness captures metrics from live runs (#435) (#451)
...
- feat: enforce the high-risk approval gate on gitlab (#448) (#449)
...
- feat(agent-process-discipline): red/green pressure-tests (#26.3-001)
...
- feat(agent-process-discipline): bugfix agents verify (#26.2-001)
...
- feat(deploy): add single-command controller + plugin deploy (#432)
...
- feat(pipeline-on-gitlab): forge-agnostic dashboard (#23.7-001)
...
- feat(pipeline-on-gitlab): gate the merge on gitlab ci (#23.2-002)
...
- feat(pipeline-on-gitlab): build opens merge requests on a (#23.2-001)
...
- feat(pipeline-on-gitlab): gitlab auth & ci tokens (#23.6-001)
...
- feat(pipeline-on-gitlab): mr/pr operation interface + (#23.1-001)
...
- fix(github-story-mirror): end story body at the next section heading
- fix(github-story-mirror): init provisions taxonomy labels first
...
- feat(github-story-mirror): resolve developer identity; (#22.5-001)

### Fixed

- fix(github-story-mirror): stamp run actor; use owner_from_issue
...
- feat(github-story-mirror): build-loop integration — (#22.4-002)
...
- feat(github-story-mirror): `sdlc issues sync` — reconcile (#22.4-001)
...
- feat(github-story-mirror): issue rendering + board/label (#22.2-002)
...
- feat(github-story-mirror): project the md specs into the (#22.1-002)
...
- feat(controller): ship config in wheel and wire harness default (#239)
...
- fix(controller): merge artifact requires landing, not a branch commit (#238)
...
- fix(controller): branch each cohort worktree from current origin main (#237)
...
- fix(controller): treat committed-but-unvalidated stage as recoverable (#235)
...
- fix(controller): tolerate fenced and trailing result-block deviations (#234)
...
- fix(harness): codex defaults to account model, not fixed ids (#230)
...
- fix(controller): embed schema-derived field skeleton in result wrapper (#229)
...
- feat(cross-harness-portability): per-harness, per-stage (#20.7-004)
...
- feat(cross-harness-portability): bring `build-stories` (#20.7-002)
...
- feat(cross-harness-portability): per-repo default harness (#20.7-005)
...
- feat(cross-harness-portability): wire per-role `--harness` (#20.7-001)
...
- fix(controller): keep dispatched agents from nested builds and browsers
...
- fix(controller): stop sdlc build committing on the checked-out branch
...
- feat(cross-harness-portability): route review/qa roles to (#20.3-002)
...
- feat(cross-harness-portability): "add a new harness" guide (#20.6-001)
- feat(cross-harness-portability): skill generator/transpiler (#20.4-002)
...
- feat(cross-harness-portability): role→harness assignment (#20.2-001)
...
- feat(cross-harness-portability): pluggable per-harness (#20.1-002)
...
- feat(cross-harness-portability): define the harness (#20.1-001)
...
- feat(cross-harness-portability): harness-neutral skill (#20.4-001)
...
- fix(controller): refresh the host registry on sdlc reconcile
