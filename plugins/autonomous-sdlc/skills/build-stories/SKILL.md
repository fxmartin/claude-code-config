...
deterministic Python in `controller/`, **not** in this prompt. Story 7.3-001
ported the state machine out of this skill. Your only job is to invoke the
controller and return its exit code.

## Invocation

Run the controller, forwarding the user's arguments verbatim:
...
Surface the controller's stdout/stderr to the user and propagate its exit code
unchanged. Do not re-implement any of the orchestration here.
