...
- **CODE_BUG** — the implementation code is wrong (wrong behavior, missing feature/case, runtime error, logic error)
...
When `{{FAILED_STEP}}` is `review` (or `{{FAILURE_OUTPUT}}` otherwise carries review
findings), **review findings are claims, not orders.** Blindly implementing a wrong
finding writes a non-bug into the codebase and burns a bounded, cost-escalating retry
cycle. Process every finding through this reception sequence:
...
1. Create a GitHub issue titled `bug({{EPIC_NAME}}): [short description] (#{{STORY_ID}})`
   (fix-issue path: `bug: [short description] (from fix #{{ISSUE_NUMBER}})`) whose body
   records the work item, branch, failed step, the failure output, your root-cause
   diagnosis, and the diagnostic checklist results (reproduce / isolated-to /
   environment check / diff-from-main), noting it was created automatically by the
   orchestrator.
...
2. Re-run to verify
3. Commit as `test({{EPIC_NAME}}): fix test for {{STORY_TITLE}}` (fix-issue path:
   `test: fix test for issue #{{ISSUE_NUMBER}}`), same trailer as above
...
5. **If no fixed version exists** and the finding is genuinely not reachable,
   ...
   pass.
6. **Commit the bump** as `fix({{EPIC_NAME}}): bump <name> to <fixed> for <OSV-ID>`.
...
that conforms to `bugfix-agent-response.schema.json` (`controller/src/sdlc/schemas/`):
