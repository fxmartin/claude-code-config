...
| Epic-11 | Realtime Progress & Multi-Run Observability | Roadmap | 16 | 56 | P2 | **IN PROGRESS (8/16)** — original 8 stories shipped through v1.34.0 (PRs #78-#83; 11.2-002/003 recovered from NEEDS_ATTENTION). Still planned: 8 dashboard stories 11.2-006…013 — GitHub repo-health panel (issues/PRs/CI), wave-column dependency DAG, live TODO/STARTED/DONE/FAILED status, in-dashboard transcript viewer, stable-height live regions, story titles on the dashboard, and surfacing fix-issue runs in the dashboard |
| Epic-12 | Controller Robustness & Failure Recovery | Roadmap | 12 | 46 | P2 | **PLANNED** ([#72](https://github.com/fxmartin/claude-code-config/issues/72)) — recover malformed result envelopes before parking, guard preflight against recursive hangs, non-destructive progress renderer, commit-message linting at commit time, auto-apply pending ledger migrations at launch; +Feature 12.3/12.4 (5 stories) from the epic-11/12 mislabel post-mortem — honest run-terminal status (reconcile vs `origin/main`, `sdlc reconcile` verb, `AWAITING_APPROVAL` state, shared finalize helper) and branch-isolation (cut branches from `origin/main`, reposition HEAD between stories); +12.5-001 dependency-line parser hardening (root-cause fix for the PR #92 hot-patch); +12.2-004 compliant-commit-subjects-by-construction (root-cause fix for the run 7df64f19 commitlint stall) |
| Epic-13 | Agent Runtime Security Hardening | Roadmap | 5 | 19 | P2 | **PLANNED** — harden the agent harness (we dispatch with `--dangerously-skip-permissions`): deny-rules for secret paths, supply-chain scan of hooks/skills/MCP, untrusted-content sanitization, kill-switch + heartbeat, optional container sandbox. From ECC `the-security-guide.md`. Created 2026-06-20 |
| Epic-14 | Cost & Model Governance | Roadmap | 6 | 23 | P2 | **PLANNED** — make consumption enforceable not just observable, tuned for the **Max subscription** ($ is notional): token-first budget gate, Max rate-limit/quota awareness with auto-resume on window reset, pre-dispatch usage estimate, per-task model routing (Balanced map: Haiku merge/discovery · Sonnet build/coverage/review · Opus high-risk + adversarial) with cheap-first escalation on retry, thinking-token cap. From ECC `the-longform-guide.md`. Created 2026-06-20 |
...
| Epic-17 | True Parallel Story Execution | Roadmap | 5 | 19 | P2 | **PLANNED** — the controller computes cohorts but executes stories sequentially (`mode=parallel` is an unused label). Add a bounded concurrent executor (default 5, `--concurrency=N`), per-story git-worktree isolation, concurrency-safe ledger writes, and truthful concurrency in status. From the epic-11 run post-mortem. Created 2026-06-20 |
...
| Epic-19 | Cross-Harness SDLC Portability | Roadmap | 13 | 52 | P2 | **PLANNED** — generalizes the controller dispatch + Epic-08 reviewer registry into a config-driven harness abstraction; per-role routing (Claude builds / Codex reviews+QAs), Codex as the first concrete adapter, single-source skill generation (kills the manual mirror drift), capability gating with safe degradation. `fix-issue`/`resume-build-agents` stay Claude-only. Created 2026-06-26 |
