...
| Epic-11 | Realtime Progress & Multi-Run Observability | Roadmap | 13 | 47 | P2 | **IN PROGRESS (8/13)** — original 8 stories shipped through v1.34.0 (PRs #78-#83; 11.2-002/003 recovered from NEEDS_ATTENTION). Still planned: 5 dashboard stories 11.2-006…010 — GitHub repo-health panel (issues/PRs/CI), wave-column dependency DAG, live TODO/STARTED/DONE/FAILED status, in-dashboard transcript viewer |
| Epic-12 | Controller Robustness & Failure Recovery | Roadmap | 10 | 40 | P2 | **PLANNED** ([#72](https://github.com/fxmartin/claude-code-config/issues/72)) — recover malformed result envelopes before parking, guard preflight against recursive hangs, non-destructive progress renderer, commit-message linting at commit time, auto-apply pending ledger migrations at launch; +Feature 12.3/12.4 (5 stories) from the epic-11/12 mislabel post-mortem — honest run-terminal status (reconcile vs `origin/main`, `sdlc reconcile` verb, `AWAITING_APPROVAL` state, shared finalize helper) and branch-isolation (cut branches from `origin/main`, reposition HEAD between stories) |
