...
| Epic-20 | Cross-Harness SDLC Portability | Roadmap | 18 | 72 | P2 | **IN PROGRESS (13/18)** — original 13 merged on `main` (2026-06-27, parallel runs 68e5a36c/e4f9976c, PRs #200-#212): config-driven harness abstraction, per-role routing, Codex adapter, single-source skill generation, capability gating. **Re-opened 2026-06-27 with Feature 20.7 (5 stories)** after cross-harness usage testing found three gaps shipped incomplete: per-role `--harness` routing was preflight/ledger-label only (still ran `claude`), the Codex `build-stories` was never moved to the single-source generator, and per-stage model routing is Claude-only; plus a per-repo `.sdlc-harness.yaml` default. `fix-issue`/`resume-build-agents` stay Claude-only. Created 2026-06-26 |
