...
| Epic-20 | Cross-Harness SDLC Portability | Roadmap | 18 | 72 | P2 | **COMPLETE (18/18)** — original 13 merged on `main` (2026-06-27, parallel runs 68e5a36c/e4f9976c, PRs #200-#212): config-driven harness abstraction, per-role routing, Codex adapter, single-source skill generation, capability gating. **Feature 20.7 (5 stories)** added 2026-06-27 after cross-harness usage testing found three gaps shipped incomplete: per-role `--harness` routing was preflight/ledger-label only (it *labelled* the ledger but still ran `claude`), the Codex `build-stories` was never moved to the single-source generator, and per-stage model routing was Claude-only; plus a per-repo `.sdlc-harness.yaml` default. **20.7-001 wired routing into real dispatch** (a codex-routed stage dispatches the Codex adapter, not just a ledger label); all five 20.7 stories merged (PRs through #223). `fix-issue`/`resume-build-agents` stay Claude-only. Created 2026-06-26 |
