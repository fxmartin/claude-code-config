...
| Epic-20 | Cross-Harness SDLC Portability | Roadmap | 13 | 52 | P2 | **COMPLETE (13/13)** — all stories merged on `main` (2026-06-27, parallel runs 68e5a36c/e4f9976c, PRs #200-#212): config-driven harness abstraction over controller dispatch + the Epic-08 reviewer registry; per-role routing (Claude builds / Codex reviews+QAs), Codex as the first concrete adapter, single-source skill generation (kills the manual mirror drift), capability gating with safe degradation. `fix-issue`/`resume-build-agents` stay Claude-only. Created 2026-06-26 |
