...
| Epic-21 | Controller Self-Containment & Harness Selection | Roadmap | 3 | 8 | P2 | **CODE-COMPLETE (2/3)** — 21.1-001 ship config in the wheel (installed `sdlc` resolves its registry; was `None`) + 21.2-001 registry-level default-harness selector with `flag > repo file > registry default > claude` precedence and a commented claude/codex toggle, merged on `main` 2026-06-28 (PR #239). 21.3-001 (installer puts the Codex adapter on PATH) PLANNED — edits `install.sh` so it must clear the high-risk gate. Surfaced wiring a repo to build on Codex. Created 2026-06-28 |
