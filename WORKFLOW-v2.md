...
> v2 mirrors run lifecycle milestones to Telegram so long-running autonomous runs can be monitored away from the machine.
...
Long-running autonomous runs mirror lifecycle milestones to Telegram so they can be monitored away from the machine:

- **Skills** call `~/.claude/hooks/notify-telegram.sh "<title>" "<body>"` at lifecycle milestones (e.g. fix-issue started/complete, requirements/stories/epic created).
- **The controller** (`sdlc build`/`resume`) emits run-lifecycle notifications directly via `notify.py` (run started / finished / rate-limited / story failed).

Both paths are best-effort and Telegram-only: credentials come from `$TELEGRAM_BOT_TOKEN` / `$TELEGRAM_CHAT_ID` (env first, then `~/.claude/config/.env`), and every call is a silent no-op when unconfigured. There are no desktop or sidebar notifications.
...
| Moment | Notification |
|--------|-------------|
| Build starts | "Build Stories Started" |
...
| Phase | What happens |
|-------|-------------|
| 1. Validate | Parse args, check git state, pre-flight tests; Telegram: "Fix Issue Started" |
| 2. Fetch | Pull issue from GitHub, validate state |
| 3. Investigate | Root cause analysis via investigation agent |
| 4. Build | TDD fix via specialized build agent |
| 5. Coverage | Coverage gate ensures 90%+ |
| 6. Review | Senior code reviewer validates fix |
| 7. E2E | End-to-end tests (if enabled) |
| 8. Bugfix | Auto-fix gate failures (max 2 retries) |
| 9. Merge | PR merge, issue close |
| 10. Summary | Generate fix report |
| 11. Done | Telegram: "Fix Issue Complete" |
...
Skill notifications flow through a single standalone hook:

```
Skill
    |
    v
~/.claude/hooks/notify-telegram.sh "Title" "Body"
    |
    +---> curl Telegram API     (if TELEGRAM_BOT_TOKEN / TELEGRAM_CHAT_ID configured;
                                 env first, then ~/.claude/config/.env)
```

The controller (`sdlc build`/`resume`) emits its own run-lifecycle notifications via `notify.py`. Both paths are best-effort and a silent no-op when unconfigured. No direct Telegram `curl` blocks exist in any workflow skill. The dedicated `/telegram` skill remains available for ad-hoc messages.
